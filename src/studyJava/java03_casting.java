...
public class java03_casting {
	...
	public static void main(String[] args) {
		...
		int i = 100;
		double d = i; //자동 형변환 : 4바이트크기 정수값 100을 8바이트 크기 에 자동 형변환 하여 저장
		...
		double db = 3.99;
		//int in = db; //에러. 왜?? db 8바이트 큰 물건을 4바이트 크기 in 방에 저장할 수 없다
		int in = (int)db; //in에 저장하는 값은??? 3이 저장되고 데이터 손실이 발생된다(반올림 하지 않는다)
		...
		int c11 = c1; //문자를 정수로 바꿔서 저장이 가능한가? yes. 모든 문자는 ASCII 코드 값
		int c22 = c2;
		...
		//숫자->문자
		int num=66;
		char c33 = (char)num; //66을 char형식으로 강제 형변환 하여 c33에 저장
		...
		//double형으로 형변환후 나누면 정확한 결과를 구할 수 있다
